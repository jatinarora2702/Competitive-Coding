...
class graph{
	...
	int dfs_visit(int s, int *visited, int p);	
public:
	graph(int v_, int f_);
...
int graph::dfs_visit(int s, int *visited, int p){
	int v;
	visited[s] = 1;
	list < pair < int , int > > :: iterator it;
	for(it = adj[s].begin() ; it != adj[s].end() ; it++) {
		if((visited[(*it).first] != 0) && ((*it).first != p)){
			return 1;
		}
		else if(visited[(*it).first] == 0){
			v = dfs_visit((*it).first, visited, s);
			if(v == 1)
				return 1;
		}
	}
